...
static int adf4360_set_freq(struct adf4360_state *st, unsigned long rate)
{
	...
	if ((st->clkin_freq == 0) || (st->clkin_freq > ADF4360_MAX_REFIN_RATE))
		return -EINVAL;
			     ...
			     long mask)
{
	...
	switch (mask) {
	case IIO_CHAN_INFO_FREQUENCY:
		if ((val < st->vco_min) || (val > st->vco_max))
			ret = -EINVAL;
		else
...
static int adf4360_parse_dt(struct adf4360_state *st)
{
	...
	ret = device_property_read_u32(dev, "adi,power-up-frequency", &tmp);
	if (ret == 0)
		st->power_up_frequency = tmp;
	else
		st->power_up_frequency = 0;
